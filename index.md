...
<div id="site-stats" style="text-align:center; margin-bottom:30px; font-size:1.1em; color:#888; background:#f5f5f5; padding:15px 10px; border-radius:10px;">
  ...
  <p>
    ...
    最近更新：<span id="latest-update">N/A</span>
  </p>
</div>
...
const postsData = [
  ...
  {
    content: `{{ post.content | markdownify | strip_html | escape }}`,
...
let totalWords = 0;
postsData.forEach(p=>{
  const cleanText = p.content.replace(/\s+/g,"");
...
for(const cat in catMap){
  ...
  const titleSpan=document.createElement('strong'); titleSpan.textContent=cat;
  ...
  for(const subcat in catMap[cat]){
    ...
    li.addEventListener('click',(e)=>{
      ...
      postsArr.forEach((p,i)=>{
        ...
        const a=document.createElement('a'); a.href=p.url; a.textContent=p.title; a.style.textDecoration='underline'; a.style.color='#06f';
        pLi.appendChild(a); postList.appendChild(pLi);
      });
